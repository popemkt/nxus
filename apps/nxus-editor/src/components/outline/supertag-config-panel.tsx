...
function SettingsTab({
  ...
  const handleChangeBaseType = useCallback(
    async (baseType: BaseType | null) => {
      const { updateSupertagConfigServerFn } = await import('@/services/supertag.server')
      await updateSupertagConfigServerFn({
        data: { supertagId: config.id, baseType },
      })
      onConfigChange({
        ...config,
        baseType,
  ...
  return (
    <div className="px-2 py-2 space-y-3">
      ...
      {/* Base type */}
      <SettingRow icon={<ChartBar size={13} weight="bold" />} label="Base type">
        <Select
          value={config.baseType ?? 'none'}
          onValueChange={(value) => {
            handleChangeBaseType(value === 'none' ? null : (value as BaseType))
          }}
        >
          <SelectTrigger className="w-full" size="sm" data-testid="supertag-base-type-select">
            <SelectValue>
              {config.baseType
