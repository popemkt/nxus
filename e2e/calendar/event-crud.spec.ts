...
test.describe('Calendar Event CRUD (CA4-CA7)', () => {
  ...
  test('CA4 — Create event via New Event button', async ({ page }) => {
    ...
    const eventOnCalendar = await findEventOnDayView(page, eventTitle)
    await expect(eventOnCalendar.first()).toBeVisible({ timeout: 10000 })
  })

  test('CA5 — View event details', async ({ page }) => {
    const eventTitle = `E2E View ${Date.now()}`
    await ensureCalendarVisible(page)

    // Create an event for this test
    await createEvent(page, { title: eventTitle, startTime: '10:00', endTime: '11:00' })

    // Find the event on Day view
    const eventOnCalendar = await findEventOnDayView(page, eventTitle)
    ...
    // Verify the modal shows time information
  ...
  test('CA6 — Edit event', async ({ page }) => {
    const eventTitle = `E2E Edit ${Date.now()}`
    const editedTitle = `${eventTitle} (edited)`
    await ensureCalendarVisible(page)

    // Create an event for this test
    await createEvent(page, { title: eventTitle })

    // Find and click the event on Day view
    const eventOnCalendar = await findEventOnDayView(page, eventTitle)
    await eventOnCalendar.first().click()
  ...
  test('CA7 — Delete event', async ({ page }) => {
    const eventTitle = `E2E Delete ${Date.now()}`
    await ensureCalendarVisible(page)
    ...
    await expect(
      page.getByText(new RegExp(`Are you sure you want to delete.*${eventTitle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`))
    ).toBeVisible()

    // Confirm deletion by clicking the destructive "Delete" button
    ...
    if (await calendarContainer.isVisible()) {
      const removedEvent = page.locator('.rbc-event').filter({ hasText: eventTitle })
