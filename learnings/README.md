...
| [e2e-db-split-brain.md](e2e-db-split-brain.md) | Playwright webServer + SQLite | Config-time `rmSync` of the e2e DB unlinks it under reused dev servers → server and tests operate on two databases at one path; reuse now defaults OFF and must stay coupled to the rm. |
