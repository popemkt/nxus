...
- **No checkbox/todo mechanism exists in the node engine today.** There is no first-class "done" boolean on `#Task` or any base-type. `todoState` therefore falls back to an engine-internal marker field, `field:tif_todo_state` (values `'todo'|'done'` verbatim) — this is TIF-specific scaffolding, not a new engine feature, and should be revisited if/when the editor gets a real checkbox/todo primitive.
