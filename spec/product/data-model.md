...
3. **Supertag inheritance** — a property with field `field:extends` on a *supertag node*, valued with the parent supertag's UUID. Chains of `extends` form the inheritance hierarchy (e.g. `#Tool extends #Item`, bootstrap.ts:267-317).
...
**Inheritance merge** (`assembleNodeWithInheritance`, node.service.ts:539-598): for each of the node's supertags, walk the `extends` chain root→leaf and merge in schema-declared fields *with non-null defaults* that the node does not already have (matching on `fieldSystemId`, :548-555,573). Own values always win over any supertag default; child-supertag defaults win over ancestor defaults. Fields declared with `null` default (pure declarations) contribute nothing to assembly — they exist for UI schema display only.

Formula fields declared by a node's supertags are the exception to the "null declarations contribute nothing" rule: when a declared field's `field:field_type` is `formula`, assembly MUST append one computed `PropertyValue` for that field using the expression stored on the field definition's `field:formula` property (`node.service.ts:388,528,535-619`). The computed value participates in the same assembled `properties` map as stored values so all read surfaces see the same value. A formula field MUST read only stored, non-formula values on the same assembled node in the MVP; referencing a formula field is a cycle error even when the reference graph would be acyclic.

**Ancestor walk depth**: the `extends` chain is followed up to 10 levels with cycle protection (`getAncestorSupertags`, node.service.ts:122-164). Query-side descendant expansion MUST use the same effective depth (invariant I4 — currently violated).
...
- **I4 — Inheritance symmetry** ✗: The supertag hierarchy depth honored when *assembling* inherited fields (ancestor walk) and when *querying* by supertag with `includeInherited` (descendant expansion) MUST be equal. *Test*: `#A ← extends ← #B ← extends ← #C`; tag node with `#C`; query for `#A` with inheritance → node matches, and the node's assembly inherits `#A`'s defaults. Currently ancestor walk = 10 levels, descendant expansion = 1 level (DRIFT: inheritance-asymmetry).
...
DRIFT: inheritance-asymmetry
- canonical: invariant I4 — assembly ancestor depth == query descendant depth (10, with cycle guard).
- current: `getAncestorSupertags` walks up to 10 levels (node.service.ts:122-164); `getNodeIdsBySupertagWithInheritance` expands descendants exactly 1 level (:919-931). A `#C extends #B extends #A` instance matches queries for `#B` but not `#A`.
- impact: supertag queries (and everything built on them: saved queries, live queries, automations, workbench views) silently miss grandchild-tagged nodes.
- closes: recursive descendant expansion with the same depth/cycle guard; I4 test.
