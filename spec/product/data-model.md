...
This single relation encodes five distinct semantics, discriminated only by which field the property uses and how consumers interpret the value:
...
5. **Inline mentions** — `field:mentions` is a system `nodes`-typed field (`SYSTEM_FIELDS.MENTIONS`/`FIELD_NAMES.MENTIONS`, node-schema.ts:186,323; seeded `fieldType: 'nodes'`, bootstrap.ts:593) whose rows are *extracted*, never authored directly. A node's `content` MAY carry inline reference tokens `[[node:<uuid>]]`; on every content write (`createNode`, `updateNodeContent`) the canonical write path re-derives the set of referenced ids from the current content and reconciles `field:mentions` against the previous set — adding rows for newly-appearing ids, deleting rows for ids whose token is gone, leaving unchanged ids untouched (`extractMentionedNodeIds`/`reconcileMentions`, `node.service.ts:740-836`, called at `:893,931`). Because it is a `nodes`-typed field, `field:mentions` rows are ordinary node references (§3.4) and need no separate backlink mechanism: the existing `linksTo` relation query finds them like any other reference. Consumers classifying backlinks by field (e.g. `getGroupedBacklinks`) MUST special-case `field:mentions` as origin kind `inline-mention` rather than a generic field-value reference, so it surfaces as "Mentioned" rather than double-counting under "Referenced" (`libs/nxus-node-api/src/server/operations.ts:572-578`). `field:mentions` is engine-internal: it is excluded from every supertag's instance schema (never appears in `supertagFieldSchemas`, same treatment as `field:extends`/`field:parent`/`field:order`) and hidden from field-row rendering (`HIDDEN_FIELD_SYSTEM_IDS`, `apps/nxus-editor/src/types/outline.ts:107`).
...
**Todo state (checkbox)** is per-node metadata: `field:todo_state` / read key `todoState`, valued `'todo' | 'done'` (`types/todo-state.ts`; seeded in `bootstrap.ts` under the same dual-key parity contract as every system field). Three states total: a node with NO `todoState` property is not a todo at all; `'todo'` renders an unchecked box, `'done'` a checked one with struck-through content (editor: `node-block.tsx`, checkbox between bullet and content, hidden from field rows via `HIDDEN_FIELD_SYSTEM_IDS`). Typing `[] ` or `[x] ` at the start of a node converts it to a todo and strips the marker. Toggling writes through the ordinary `setProperty` path so live queries and automations observe it like any property change. Calendar task completion honors `todoState === 'done'` first and falls back to the legacy free-string `field:status` heuristic only when `todoState` is absent (`calendar.server.ts`). TIF `todoState` round-trips through this same field ([tif-interchange.md](tif-interchange.md)).

**Day nodes (daily notes)** are ordinary nodes tagged `#Day` (`supertag:day`, seeded with base type `day` — `bootstrap.ts` entity list). Identity is deterministic per calendar date: systemId `item:day-YYYY-MM-DD`, so the UNIQUE systemId column — not query-then-create — guarantees at most one day node per date (a concurrent double-create races into the constraint; the loser re-reads the winner). Day nodes carry `field:start_date` = the ISO date and `field:all_day` = true, so they surface on the calendar like any dated node. A soft-deleted day node **resurrects on revisit** (`restoreNode`): the date identity is permanent, and leaving it deleted would brick that date behind the unique constraint. Entry point: the editor's Today button (`getOrCreateDayNodeServerFn`, `outline.server.ts`) — the date is computed from the CLIENT's clock (local-first: client and server share a machine; a future multi-timezone deployment must revisit this). New day nodes are seeded with one empty child so the zoomed daily page opens ready to type.

**Supertag base type** is optional metadata on a supertag definition node: `field:base_type` / read key `baseType`, valued with one of `task | person | event | day | flashcard` (`node-schema.ts:180-185,316-324`; `types/base-type.ts:3`). A base type lets a user-defined supertag opt into engine behavior without being one of the hardcoded `SYSTEM_SUPERTAGS`. Resolution MUST include a node when any assigned supertag, or any ancestor supertag reached through `field:extends`, has the requested base type; the request-scoped helper is `getNodeIdsBySupertagBaseType`/`getNodesBySupertagBaseType` and accepts the same optional trailing `AssemblyCache` pattern as other assembly helpers (`node.service.ts:1456-1558`). System `#Task` and `#Event` are bootstrapped with implicit `task`/`event` base types so existing calendar behavior is unchanged (`bootstrap.ts:309-310`).
