...
- **Configuration (full page)**: zooming into a supertag definition node replaces the outline with `SupertagDetailView` — header (name, color, zero-or-more `Extends` parent pills), a Fields tab (the tag's field schema) and a Settings tab, then a References section listing tagged instances (`supertag-detail-view.tsx:88-193`; dispatch `outline-editor.tsx:456-459,471`). Field definition nodes get the analogous `FieldDetailView` (`outline-editor.tsx:458-459,474`). Extends pills are plural because the data model allows a supertag to extend multiple parents at once; the pill order follows `field:extends` property order from [data-model.md](./data-model.md).
- **Configuration (inline popover)**: every supertag badge (`node-content.tsx` `SupertagBadges`) carries `data-supertag-badge={supertagId}` and a hover-revealed gear icon alongside the existing hover-`X` remove affordance. Clicking the gear opens `SupertagConfigPanel` (`supertag-config-panel.tsx`) as a portal-rendered popover anchored at the badge — a compact Fields/Settings tabbed editor (add/remove/retype fields, constraints, color, base type, multi-parent extends, default child, content template) backed by the same `supertag.server.ts` functions `SupertagDetailView` uses, so edits from either surface stay consistent. The Settings tab MUST expose a base-type Select (`None`, `Task`, `Person`, `Event`, `Day`, `Flashcard`) that writes `field:base_type`; both inline and full-page settings use the same server function and Select control (`supertag-config-panel.tsx:796-808,862-889`; `supertag-detail-view.tsx:468-497`). The extends picker (`supertag-config-panel.tsx:742-763,820-832`) MUST allow adding/removing multiple parents, MUST prevent selecting the current supertag, and MUST reject or disable choices that would create an `extends` cycle. This gives quick in-place configuration without leaving the outline, while `SupertagDetailView` remains the full-page editor for deeper work.

Proof: `outline-editor.spec.ts` — "Supertag Display" (:254-270), "Supertag Configuration" (:675-695, now exercises real badges/gear rather than vacuously passing), command-palette supertag step (:555-579); `formula-fields.spec.ts` — "multi-parent supertag inheritance shows fields from both parent supertags".
