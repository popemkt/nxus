...
- current: `QuerySubscriptionService.subscribe` takes `type Database = any` (query-subscription.service.ts — also violating the repo's own no-`any` rule, [../rules/](../rules/)) and passes it to the sync-SQLite `evaluateQuery`; computed-field and automation services type it as `ReturnType<typeof getDatabase>` and call sync `node.service` functions directly. `SurrealBackend` dutifully emits events that no subscription can evaluate against — graph mode has emission but no reaction.
- impact (narrowed 2026-07-11): under `ARCHITECTURE_TYPE=graph` the layer used to *silently evaluate against a disconnected SQLite copy* — wrong data, no error. It now **fails fast**: `assertReactiveSupported()` (`reactive/reactive-support.ts`) throws `ReactiveUnsupportedArchitectureError` at `computedFieldService.initialize`, `automationService.initialize`, and `queryService.subscribe`; the inbox feature boundary catches exactly that error, warns once, and returns failure envelopes. Graph mode is loudly reactive-less, never wrongly reactive.
- closes: route evaluation through `NodeBackend.evaluateQuery` (async), type `db` as the backend interface, delete the `any`, then drop the guard.

Behavior clauses (guard, guarded by same-code test titles in `reactive/__tests__/reactive-support.test.ts`):

- **REACT-B1** — Given `ARCHITECTURE_TYPE=graph`, when reactive initialization or subscription is attempted, then it MUST throw `Reactive layer is SQLite-only; unsupported under ARCHITECTURE_TYPE=graph (see spec/tech/persistence.md §5 DRIFT)` — never evaluate against a database the selected backend does not write to.
- **REACT-B2** — Given `ARCHITECTURE_TYPE` is `node` or unset, when reactive services initialize and subscribe, then behavior is unchanged by the guard.

**Membership invalidation narrowing** (canonical, closed 2026-07-11): `node:created`/`node:deleted` events carry `supertagIds` — the node's supertag UUIDs expanded through `getAncestorSupertags` at EMISSION time (`node.service.ts` `expandNodeSupertagIds`; create emits after supertag assignment, delete reads supertags before the soft-delete, restore re-expands). Registration (`extractQueryDependencies`) adds the broad `NODE_MEMBERSHIP` dependency only to subscriptions WITHOUT a required top-level supertag conjunct; subscriptions with one rely on their `supertag:<id>` keys, which enriched events hit through the reverse index — a node tagged Y invalidates a subscription on X when Y `extends` X because the EVENT carries X (emission-time expansion is load-bearing: descendant sets change when tags re-extend, so registration-time expansion would go stale). Enriched events never add `ANY_SUPERTAG` (that marker compensates for unexpanded `supertag:added` events only). Unenriched membership events — any emitter not yet passing `supertagIds` — resolve to the `AFFECTS_ALL` marker and re-evaluate every subscription, the safe legacy fallback. Proven by `reactive/__tests__/membership-narrowing.test.ts`.
