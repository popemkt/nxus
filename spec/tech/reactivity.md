...
DRIFT: membership-affects-all
- canonical: `node:created`/`node:deleted` invalidate only subscriptions whose results the node could affect: subscriptions with a required top-level supertag conjunct are invalidated iff the node's supertags intersect the filter's supertag *including inheritance* (a node tagged Y must invalidate a subscription on X when Y `extends` X); subscriptions without a required supertag conjunct keep the conservative membership dependency.
- current: `MEMBERSHIP_MUTATION_TYPES` short-circuits to `affectsAll` (`reactive/dependency-tracker.ts:283-303,419-426`) and `extractQueryDependencies` adds `NODE_MEMBERSHIP` to every subscription (`:247-249`) — every create/delete re-evaluates every live query.
- impact: query-heavy workspaces pay O(subscriptions) evaluations per node create/delete; the editor's trailing-convergence client fix (editor-sync.md) bounds the UI cost but server-side subscriptions still storm.
- closes: enrich `node:created`/`node:deleted` events with the node's supertag ids expanded through `getAncestorSupertags` at emission (emitters have db access; tracker stays pure); registration adds `NODE_MEMBERSHIP` only when no top-level supertag conjunct exists; tracker falls back to `affectsAll` for unenriched events. The inheritance expansion at EMISSION (not registration) is load-bearing — descendant sets change when tags re-extend, so registration-time expansion would go stale.
