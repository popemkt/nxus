...
Boot model: the `webServer` block runs `pnpm dev` — all 6 apps — and polls the gateway's `/__health` (`playwright.config.ts:25-32`, endpoint at `apps/nxus-gateway/vite.config.ts:29`), regardless of which single spec you asked for. `reuseExistingServer: !process.env.CI` means a local `pnpm e2e` attaches to an already-running `pnpm dev`… unless `predev` kills it first (DRIFT §1).
