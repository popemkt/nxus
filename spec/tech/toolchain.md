...
Boot model: the `webServer` block deletes the e2e DB and runs `pnpm dev` — all 6 apps — in one command, then polls the gateway's `/__health` (`playwright.config.ts`, endpoint at `apps/nxus-gateway/vite.config.ts:29`), regardless of which single spec you asked for. The delete lives INSIDE the command because the config file is re-evaluated by every worker process — a module-scope delete unlinks the DB mid-run under the live servers ([learnings/e2e-db-split-brain.md](../../learnings/e2e-db-split-brain.md)). Server reuse is OFF by default for the same reason; `PW_REUSE_SERVER=1` re-enables it for fast local iteration (no fresh delete — the DB accumulates). After boot, `e2e/global-setup.cjs` drives a real browser to trigger the server's demo auto-seed and blocks until `item:%` rows are visible, so seed presence never depends on spec order ([learnings/e2e-autoseed-suppression.md](../../learnings/e2e-autoseed-suppression.md)). The apps run with `NXUS_E2E=1`, which disables vite's error overlay (it intercepts pointer events and converts transient dev-server hiccups into unrelated click failures).
...
- current: closed 2026-07-08. The "order dependence" was three stacked infra defects, not test-state leakage: (1) the config-time DB delete ran in **every worker process** and under reused servers, splitting server and tests onto two databases at one path (moved into the `webServer` command — exactly once, before boot; [learnings/e2e-db-split-brain.md](../../learnings/e2e-db-split-brain.md)); (2) demo auto-seed depended on which spec's request reached the server first — `e2e/global-setup.cjs` now triggers the seed with a real browser and gates the run on `item:%` rows existing; (3) vite's error overlay intercepted clicks after transient dev-server hiccups — disabled under `NXUS_E2E`.
- impact: full suite at 1 worker (CI-equivalent): 86 passed / 0 failed.
- closes: closed as above; if a spec fails order-dependently again, suspect real test-state leakage and apply seeded isolation.
...
- now: the leg runs against a real SurrealDB **server** (embedded surrealkv corrupts under the six-app multi-process topology — [learnings/surrealkv-multiprocess.md](../../learnings/surrealkv-multiprocess.md)); `ci.yml` starts `surreal v3.2.1` with the `memory` engine (pinned to the SDK dialect: `surrealdb@^2.0.4` + `@surrealdb/node@^3.0.3` since the same-day v3 upgrade) and sets `SURREAL_EMBEDDED=false`/`SURREAL_URL` before seed + suite. Verified locally 2026-07-11 on the v3 stack: graph leg 61 passed / 0 failed / exit 0 (1.4 min), node leg unchanged.
