...
DRIFT: most consumers bypass the facade
- canonical: features call `nodeFacade` only; sync SQLite functions are backend-internal.
- current: only ~10 files use `nodeFacade`. The editor's server functions import the sync SQLite-only API directly — `assembleNode`, `createNode`, `setProperty`, `getDatabase`, … (`apps/nxus-editor/src/services/outline.server.ts:16-27,54,307-315`); `apps/nxus-core/src/services/nodes/index.ts:16-35` re-exports the whole sync API "for backward compatibility". The reactive layer is also SQLite-hard-wired (`reactive/query-subscription.service.ts` takes `type Database = any`).
- impact: the editor (and most of core) **breaks entirely under `ARCHITECTURE_TYPE=graph`**, while CI runs a graph e2e matrix leg (`.github/workflows/ci.yml:74-78`) that cannot actually exercise those apps; the backend abstraction is aspirational.
- closes: converge editor + core node services onto `nodeFacade` (see decision record §6); abstract or explicitly scope the reactive layer to node mode.
